...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
        if self.use_batchnorm:
            layers[i], layer_caches[i] = self.affine_batchnorm_relu_forward(layers[i-1],
                                                            self.params['W%d' % i],
                                                            self.params['b%d' % i],
            ...
            layers[i], layer_caches[i] = affine_relu_forward(layers[i-1], 
                                                             self.params['W%d' % i], 
                                                             self.params['b%d' % i])
        if self.use_dropout:
            layers[i], dropout_caches[i] = dropout_forward(layers[i], self.dropout_param)

    scores, scores_cache = affine_forward(layers[self.num_layers-1], 
    ...
    dx, grads['W%d'%self.num_layers], grads['b%d'%self.num_layers] = affine_backward(dx, scores_cache)
    ...
    for i in range(self.num_layers-1, 0, -1):
        ...
        if self.use_batchnorm:
            dx, grads['W%d'%i], grads['b%d'%i], grads['gamma%d'%i], grads['beta%d'%i] = self.affine_batchnorm_relu_backward(dx, layer_caches[i])
        else:
            dx, grads['W%d'%i], grads['b%d'%i] = affine_relu_backward(dx, layer_caches[i])
  ...
  def affine_batchnorm_relu_forward(self, x, w, b, gamma, beta, bn_params):
    """
    ...
    - gamma, beta, bn_params: params for the barch normalization layer
