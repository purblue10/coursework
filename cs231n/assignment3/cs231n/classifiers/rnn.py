...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    ### Forwad pass ###
    # Affine transformation for h0 from the image feature
    W_proj, b_proj = self.params['W_proj'], self.params['b_proj']
    affine_out, affine_cache = affine_forward(features, W_proj, b_proj)
    
    # Word embedding with captions_in
    W_embed = self.params['W_embed']
    ...
    # Softmax loss layer
    loss, dx = temporal_softmax_loss(temp_affine_out, captions_out, mask, verbose=False)
    
    ### Backward pass ###
    # Temporal affine layer
    dx, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dx, temp_affine_cache)
  ...
  def sample(self, features, max_length=30):
    ...
    for i in range(1,max_length):
      embedded_word, _ = word_embedding_forward(captions[:,i-1], W_embed)
      print embedded_word.shape
      next_h, _ = rnn_step_forward(embedded_word, prev_h, Wx, Wh, b)
      temp_affine_out, _ = temporal_affine_forward(next_h, W_vocab, b_vocab)
      print temp_affine_out
