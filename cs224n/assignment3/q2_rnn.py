...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if pad_size > 0:
            ret.append((sentence + [zero_vector]*pad_size, 
                labels + [zero_label]*pad_size, 
                [True]*len(sentence) + [False]*pad_size))
        else:
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder = tf.placeholder(tf.int32, shape=(None, config.max_length))
        self.mask_placeholder = tf.placeholder(tf.bool, shape=(None, config.max_length))
    ...
    def add_embedding(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        config = self.config
        embed = tf.get_variable("embed", initializer = self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(embed, self.input_placeholder)
        embeddings = tf.reshape(embeddings, [-1, config.max_length, config.n_features * config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                y_hat = tf.matmul(output_drop, U) + b2
                preds.append(y_hat)
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        losses = tf.nn.sparse_softmax_cross_entropy_with_logits(labels = y_masked, logits = preds_masked)
