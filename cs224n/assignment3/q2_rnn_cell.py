...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable("W_x", shape=[self.input_size, self._state_size], initializer = tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
            W_h = tf.get_variable("W_h", shape=[self._state_size, self._state_size], initializer = tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
            b = tf.get_variable("b", shape=[self._state_size], initializer = tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
