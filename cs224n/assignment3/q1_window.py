...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(T):
            x = [item for sublist in sentence[i:i+2*window_size+1] for item in sublist]
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("baseline"):
            W = tf.get_variable("W", shape=[config.n_window_features * config.embed_size, config.hidden_size], initializer = tf.contrib.layers.xavier_initializer())
    ...
    def add_loss_op(self, pred):
        ...
        losses = tf.nn.sparse_softmax_cross_entropy_with_logits(labels = self.labels_placeholder, logits=pred)
